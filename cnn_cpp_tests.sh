...
#
# Matthew Abbott 2025
# CNN C++ Tests - COMPLETE port from Pascal test_cnn.sh
# Tests both regular C++ and OpenCL implementations
# Includes ALL tests from original Pascal suite
#
...
# Compile C++ binaries
echo "Compiling C++ CNN binaries..."
g++ -std=c++11 -O2 cnn.cpp -o "$CNN_BIN" 2>&1
if [ $? -ne 0 ]; then
    echo -e "${RED}Failed to compile cnn.cpp${NC}"
    exit 1
fi

g++ -std=c++11 -O2 facaded_cnn.cpp -o "$FACADE_BIN" 2>&1
if [ $? -ne 0 ]; then
    echo -e "${RED}Failed to compile facaded_cnn.cpp${NC}"
    exit 1
fi
...
echo ""
echo "========================================="
echo "CNN C++ User Workflow Test Suite"
echo "========================================="
echo ""

# Check binaries exist
if [ ! -f "$CNN_BIN" ]; then
    echo -e "${RED}Error: $CNN_BIN not found${NC}"
    exit 1
fi

if [ ! -f "$FACADE_BIN" ]; then
    echo -e "${RED}Error: $FACADE_BIN not found${NC}"
    exit 1
fi

echo -e "${BLUE}=== CNN C++ Binary Tests ===${NC}"
echo ""
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/basic_cnn.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10" \
    "Creating"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/cnn_test1.json --input=32,32,3 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10" \
    "Input: 32x32x3"

run_test \
    "Output shows correct input height" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_test2.json --input=64,64,1 --conv=32 --kernels=3 --pool=2 --fc=256 --output=5" \
    "Input: 64x64x1"

run_test \
    "Output shows correct channels" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_test3.json --input=28,28,3 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10" \
    "Input: 28x28x3"

run_test \
    "Output shows correct output size" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_test4.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=5" \
    "Output: 5"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/multi_conv.json --input=28,28,1 --conv=16,32,64 --kernels=3,3,3 --pool=2,2,2 --fc=256,128 --output=10" \
    "Creating"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/multi_conv2.json --input=28,28,1 --conv=32,64 --kernels=3,3 --pool=2,2 --fc=128 --output=10" \
    "Creating"

run_test \
    "Multi-conv with varying kernel sizes" \
    "$CNN_BIN create --model=$TEMP_DIR/multi_conv3.json --input=28,28,1 --conv=16,32 --kernels=3,5 --pool=2,2 --fc=128 --output=10" \
    "Creating"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/small_cnn.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=64 --output=10" \
    "Creating"

run_test \
    "Medium CNN (32,64 filters)" \
    "$CNN_BIN create --model=$TEMP_DIR/medium_cnn.json --input=32,32,3 --conv=32,64 --kernels=3,3 --pool=2,2 --fc=256,128 --output=10" \
    "Creating"

run_test \
    "Large CNN (64,128 filters)" \
    "$CNN_BIN create --model=$TEMP_DIR/large_cnn.json --input=28,28,3 --conv=64,128 --kernels=3,3 --pool=2,2 --fc=512,256 --output=10" \
    "Creating"

run_test \
    "Different input channels (grayscale)" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_gray.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10" \
    "Input: 28x28x1"

run_test \
    "Different input channels (RGB)" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_rgb.json --input=28,28,3 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10" \
    "Input: 28x28x3"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/cnn_lr1.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --lr=0.01" \
    "Learning rate: 0.01"

run_test \
    "Custom learning rate (0.001)" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_lr2.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --lr=0.001" \
    "Learning rate: 0.001"

run_test \
    "Custom gradient clip" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_clip.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --clip=10.0" \
    "Gradient clip: 10"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/cnn_relu.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --activation=relu" \
    "Hidden activation: relu"

run_test \
    "Sigmoid hidden activation" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_sigmoid.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --activation=sigmoid" \
    "Hidden activation: sigmoid"

run_test \
    "Tanh hidden activation" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_tanh.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --activation=tanh" \
    "Hidden activation: tanh"

run_test \
    "Linear output activation" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_outlin.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --output-act=linear" \
    "Output activation: linear"

run_test \
    "Sigmoid output activation" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_outsig.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --output-act=sigmoid" \
...
run_test \
    "MSE loss (default)" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_mse.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --loss=mse" \
    "Loss function: mse"

run_test \
    "Cross-entropy loss" \
    "$CNN_BIN create --model=$TEMP_DIR/cnn_ce.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --loss=crossentropy" \
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/cross.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 && $FACADE_BIN info --model=$TEMP_DIR/cross.json" \
    "Input:"
...
run_test \
    ...
    "$CNN_BIN create --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing input width" \
    "$CNN_BIN create --model=$TEMP_DIR/err1.json --input=,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing input height" \
    "$CNN_BIN create --model=$TEMP_DIR/err2.json --input=28,,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing input channels" \
    "$CNN_BIN create --model=$TEMP_DIR/err3.json --input=28,28 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing conv filters" \
    "$CNN_BIN create --model=$TEMP_DIR/err4.json --input=28,28,1 --kernels=3 --pool=2 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing kernel sizes" \
    "$CNN_BIN create --model=$TEMP_DIR/err5.json --input=28,28,1 --conv=16 --pool=2 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing pool sizes" \
    "$CNN_BIN create --model=$TEMP_DIR/err6.json --input=28,28,1 --conv=16 --kernels=3 --fc=128 --output=10 2>&1" \
    "Error"

run_test \
    "Missing FC layers" \
    "$CNN_BIN create --model=$TEMP_DIR/err7.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --output=10 2>&1" \
    "Error"

run_test \
    "Missing output size" \
    "$CNN_BIN create --model=$TEMP_DIR/err8.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 2>&1" \
    "Error"
...
run_test \
    ...
    "$CNN_BIN create --model=$TEMP_DIR/workflow1.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 && $FACADE_BIN info --model=$TEMP_DIR/workflow1.json" \
    "Input: 28x28x1"

run_test \
    "Workflow: Create -> Load -> Info (RGB)" \
    "$CNN_BIN create --model=$TEMP_DIR/workflow2.json --input=32,32,3 --conv=32 --kernels=3 --pool=2 --fc=256 --output=10 && $FACADE_BIN info --model=$TEMP_DIR/workflow2.json" \
    "Output size: 10"

run_test \
    "Workflow: Create -> Load -> Info (Multi-layer)" \
    "$CNN_BIN create --model=$TEMP_DIR/workflow3.json --input=28,28,1 --conv=16,32,64 --kernels=3,3,3 --pool=2,2,2 --fc=256,128 --output=10 && $FACADE_BIN info --model=$TEMP_DIR/workflow3.json" \
    "Input: 28x28x1"

run_test \
    "Workflow: Create with params -> Verify -> Load" \
    "$CNN_BIN create --model=$TEMP_DIR/workflow4.json --input=28,28,1 --conv=16,32 --kernels=3,3 --pool=2,2 --fc=256,128 --output=10 --lr=0.001 --clip=5.0 && $FACADE_BIN info --model=$TEMP_DIR/workflow4.json" \
    "Input: 28x28x1"
...
run_test \
    "Large filter count" \
    "$CNN_BIN create --model=$TEMP_DIR/large_filters.json --input=28,28,1 --conv=256 --kernels=3 --pool=2 --fc=512 --output=10" \
    "Creating"

run_test \
    "Multiple convolutional layers (3 layers)" \
    "$CNN_BIN create --model=$TEMP_DIR/multi_layer.json --input=28,28,1 --conv=16,32,64 --kernels=3,3,3 --pool=2,2,2 --fc=256 --output=10" \
    "Creating"

run_test \
    "Large fully connected layer" \
    "$CNN_BIN create --model=$TEMP_DIR/large_fc.json --input=28,28,1 --conv=32 --kernels=3 --pool=2 --fc=1024 --output=10" \
    "Creating"

run_test \
    "Multiple FC layers" \
    "$CNN_BIN create --model=$TEMP_DIR/multi_fc.json --input=28,28,1 --conv=16,32 --kernels=3,3 --pool=2,2 --fc=512,256,128 --output=10" \
    "Creating"

run_test \
    "Different kernel sizes per layer" \
    "$CNN_BIN create --model=$TEMP_DIR/diff_kernels.json --input=28,28,1 --conv=16,32 --kernels=3,5 --pool=2,2 --fc=128 --output=10" \
    "Creating"

run_test \
    "High learning rate" \
    "$CNN_BIN create --model=$TEMP_DIR/high_lr.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --lr=0.1" \
    "Learning rate: 0.1"

run_test \
    "Low learning rate" \
    "$CNN_BIN create --model=$TEMP_DIR/low_lr.json --input=28,28,1 --conv=16 --kernels=3 --pool=2 --fc=128 --output=10 --lr=0.0001" \
    "Learning rate: 0"
...
if command -v node &> /dev/null; then
    ...
    $CNN_BIN create --model=$TEMP_DIR/test_weights.json --input=28,28,1 --conv=8 --kernels=3 --pool=2 --fc=64 --output=10 > /dev/null 2>&1
    ...
    run_test \
        "Weight loading: FC weight values match" \
        "node validate_weights.js $TEMP_DIR/basic_cnn.json 2>&1" \
